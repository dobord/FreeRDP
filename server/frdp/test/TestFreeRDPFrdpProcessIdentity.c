...
int TestFreeRDPFrdpProcessIdentity(int argc, char* argv[])
{
	...
	if (frdp_sesmand_process_identity_read(0, &start_ticks, &owner_uid) !=
...
#ifdef __linux__
	if (frdp_sesmand_process_identity_read(getpid(), &start_ticks, &owner_uid) !=
	        FRDP_SESMAND_PROCESS_IDENTITY_OK ||
	    (start_ticks == 0) || (owner_uid != geteuid()))
	{
		fprintf(stderr, "current process identity could not be read\n");
		return -1;
	}
	if (frdp_sesmand_process_identity_read((pid_t)INT_MAX, &start_ticks, &owner_uid) !=
