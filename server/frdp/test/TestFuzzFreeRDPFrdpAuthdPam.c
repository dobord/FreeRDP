...
#define FRDP_AUTHD_PAM_FUZZ_MAX_SIZE 512U
#define FRDP_AUTHD_PAM_FUZZ_MAX_MESSAGES 8U
#define FRDP_AUTHD_PAM_FUZZ_MAX_TEXT 32U
...
{
	struct pam_message messages[FRDP_AUTHD_PAM_FUZZ_MAX_MESSAGES] = { 0 };
	const struct pam_message* message_ptrs[FRDP_AUTHD_PAM_FUZZ_MAX_MESSAGES] = { 0 };
	char message_text[FRDP_AUTHD_PAM_FUZZ_MAX_MESSAGES][FRDP_AUTHD_PAM_FUZZ_MAX_TEXT] = { 0 };
	char password[FRDP_AUTHD_PAM_FUZZ_MAX_TEXT] = { 0 };
	struct pam_response* responses = NULL;
	size_t count = 1;
	size_t offset = 0;
	void* appdata = password;

	if (!Data || (Size > FRDP_AUTHD_PAM_FUZZ_MAX_SIZE))
		return 0;

	if (Size > 0)
	{
		count = (Data[0] % FRDP_AUTHD_PAM_FUZZ_MAX_MESSAGES) + 1U;
		offset = 1U;
	}
	...
	(void)frdp_authd_pam_conversation((int)count, message_ptrs, &responses, appdata);
	free_authd_pam_responses(responses, (int)count);
	return 0;
}
