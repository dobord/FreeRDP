...
7. In the local profile, the harness gracefully stops `frdpd`, requires its peer worker to detach the held session before the daemon exits, starts a new daemon PID while `frdp-sesmand` remains alive, and then connects the second client. Other profiles terminate the first client directly.
8. The second graphical client reattaches to the only matching session with the same session id, display and agent PID. Post-connect resynchronizes the retained `800x600` display to the new client's `1024x768` request before framebuffer pumping; its disconnect and explicit `kill-session` leave an empty registry.
