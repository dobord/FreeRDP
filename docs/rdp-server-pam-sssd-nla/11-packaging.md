...
- The resulting binary package declares runtime dependencies through `${shlibs:Depends}` / `${misc:Depends}` plus `libpam0g`, `sssd`, `xvfb`, and the Xorg dummy video driver for the PAM/SSSD session-agent backends. The server component installs `/usr/share/frdpd/xorg-dummy.conf`; deployments selecting it must configure the distro's raw Xorg executable path explicitly.
