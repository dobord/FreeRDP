...
- [x] real-client local-PAM, Samba AD/SSSD, and joined FreeIPA/SSSD profiles disable automatic client reconnect, verify fresh server-side evidence for three baseline accepted NLA/PAM attempts, one wrong-password NTLM MIC rejection before PAM, and two named disabled-account or HBAC-policy PAM/SSSD denials from that probe's connection path, clean the observed session created by the successful auth-only probe, and prove each denied attempt leaves the managed-session registry plus durable session runtime directory empty; Samba and FreeIPA additionally authenticate three recovery probes through SSSD around an active-session manager crash, while FreeIPA rotates the enrolled host principal, requires an increased KVNO, rejects the old key, accepts the atomically installed root-only replacement, and completes the RDP/PAM/SSSD probes after rollover;
