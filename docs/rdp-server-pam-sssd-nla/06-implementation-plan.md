...
cmake --build /tmp/opencode/freerdp-frdp-fuzz --target TestFuzzFreeRDPFrdpConfig TestFuzzFreeRDPFrdpAuthToken TestFuzzFreeRDPFrdpFramePolicy TestFuzzFreeRDPFrdpInputPolicy
...
- [x] cleanup on disconnect across `frdp-sesmand` agent process groups and PAM sessions (`session_socket` close requests terminate the agent process group, mark the in-memory session state through the covered `ACTIVE` -> `STOPPING` -> `DEAD` path, and close PAM state; durable cleanup after prolonged `frdp-sesmand` outage remains tracked separately).
