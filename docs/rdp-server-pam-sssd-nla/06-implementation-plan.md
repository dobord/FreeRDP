...
- [ ] protocol regression suite (partial: `server/frdp/test/e2e/scripts/rdp-protocol-regression.sh` provides a retained-client auth-only NLA option matrix with per-case logs, `rdp-session-smoke.sh` and the standard local/Samba/FreeIPA probes provide retained-client graphical disconnect/reconnect lifecycle coverage with stable session identity, and the E2E harness scripts/fixtures have focused syntax/shape validation plus bounded profile/probe timeout and client-tool prerequisite checks; broader codec/channel, restart-reconnect, Windows-client, soak, and sustained interop regression coverage remains open);
