...
- [ ] logind/cgroup integration (partial: `frdp-sesmand` applies configured POSIX `RLIMIT_NPROC` and `RLIMIT_AS` guards and can optionally hold each new child behind a launch barrier while systemd creates a transient `frdp-session-<uuid>.scope`, moves the PID into it, enables accounting, and maps the limits to `TasksMax`/`MemoryMax`. The barrier is released only after the unit reaches `ActiveState=active`; startup/reload/session creation fail closed when requested scope ownership is unavailable. Hard close waits for the unit to stop, with cgroup-v2 `cgroup.kill` and the existing process-group cleanup as bounded fallbacks. Durable metadata records scope ownership so manager-restart reconciliation derives and stops the unit before removing artifacts. Focused tests cover config/name/limit/metadata translation and a root/systemd-gated test verifies D-Bus reconnection, the live PID cgroup, properties, a detached `setsid()` descendant, restart-style reconciliation, stop, and collection. systemd-logind registration, lost-PAM-handle reconciliation, CPU quotas, and production dynamic quota management remain open);
