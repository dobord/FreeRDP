...
- [ ] simple reconnect by user/session id (partial: reconnect-selection policy for matching authenticated users to an explicit session id or the newest disconnected session now has focused CTest coverage, normal peer disconnect now sends a detach request that marks the in-memory session `DISCONNECTED`, and authorized explicit session-id V3 requests can attach to that disconnected entry instead of silently creating a fresh session; durable/user-implicit reconnect behavior and client interop evidence remain open);
