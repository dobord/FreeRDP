...
- [ ] simple reconnect by user/session id (partial: reconnect-selection policy for matching authenticated users to an explicit session id or the newest disconnected session now has focused CTest coverage, and authorized explicit session-id V3 requests can attach to a disconnected in-memory session entry instead of silently creating a fresh session; normal disconnect still closes sessions, and durable/user-implicit reconnect behavior remains open);
