...
- [x] cleanup on disconnect across `frdp-sesmand` agent process groups and PAM sessions (`session_socket` hard-close requests use covered cleanup-decision policy to terminate the agent process group, close PAM state, unlink runtime sockets, release display reservations, and mark the in-memory session state through the `ACTIVE` -> `STOPPING` -> `DEAD` path; normal peer disconnect uses a distinct detach request and falls back to a hard-close retry if the session manager cannot acknowledge the detach; Linux display reservations persist a versioned owner PID plus `/proc` start time and use a shared process-identity reader that obtains start time and owner UID from one opened proc entry, safely reconciling locks whose process disappeared or whose PID was reused, while legacy PID-only reservations remain readable and full durable session/process/PAM cleanup after prolonged `frdp-sesmand` outage remains tracked separately).
