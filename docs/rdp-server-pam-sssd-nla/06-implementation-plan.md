...
- [x] provider-backed session-manager failure coverage when `libpam-wrapper` is available (a test-only PAM module records account/credential/session phases, deterministically rejects `pam_open_session`, and proves the real `frdp-sesmand` returns a clean V3 failure without creating a session, agent socket, display reservation, or durable metadata);
