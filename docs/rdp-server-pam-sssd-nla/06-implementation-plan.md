...
- [x] auth/session helper readiness and service recovery policy: explicit role-bound health responses, bounded pre-listen checks in `frdpd`, fail-closed requests during helper outage, `Restart=on-failure` units, direct same-socket crash/restart coverage, deterministic in-flight PAM auth/session open/session close interruption coverage, a root-gated transient-systemd test that kills and restarts both helper roles while requiring a new socket inode plus role-bound health on the same path, and an installed Debian-unit gate that kills `frdp-sesmand` with an active real-client session and proves restart identity/socket replacement, orphan cleanup and fresh login; prolonged outages, installed auth-broker active-login crashes and provider-backed installed-unit recovery remain deployment concerns;
