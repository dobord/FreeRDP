...
- [ ] logind/cgroup integration (partial: `frdp-sesmand` can apply configured POSIX `RLIMIT_NPROC` and `RLIMIT_AS` guards to newly launched session agents, with focused CTest coverage for disabled policy, process/memory limit translation, overflow rejection, and fail-closed `setrlimit()` errors; systemd-logind scope ownership, cgroup CPU/memory accounting, and production quota management remain open);
