...
- [x] bounded agent heartbeat supervision on a private channel independent of the frame/input listener (configurable interval, timeout, and consecutive-failure threshold) with live PAM/Xvfb coverage proving normal control-path contention does not trigger cleanup and a stopped agent causes cleanup of its Xvfb process group, metadata, control socket, and display reservation; direct `authd` supervision and logind/cgroup ownership remain open;
