...
- [ ] simple reconnect by user/session id (partial: reconnect-selection policy for matching authenticated users to an explicit session id or the newest disconnected session now has focused CTest coverage, and explicit session-id open requests now fail closed instead of silently creating a fresh session; runtime attach/reuse behavior is still not implemented);
