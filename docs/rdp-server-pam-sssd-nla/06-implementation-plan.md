...
- `server/frdp/frdp-session-agent` (launched by normal `frdp-sesmand` session requests; keyboard/mouse/strict UTF-16 text backend injection including supplementary-plane surrogate pairs, raw framebuffer tile capture with XDamage-backed dirty-tile tracking, and validated XRandR resize-request handling exist, but the current fixed-mode Xvfb backend cannot apply geometry changes, and IME/layout-safe text input and production channel features are not implemented yet);
...
- [ ] display resize (partial: the policy-gated Display Control DVC validates client monitor layouts on its worker thread, queues them for the peer thread, forwards the resulting geometry to the agent, and updates peer geometry only after backend confirmation. The local real-client profile proves DVC capability exchange and an `800x600` monitor-layout request, but the fixed-mode Xvfb backend rejects the geometry change, so actual resize and churn remain open);
