...
- [ ] GSSAPI/Kerberos acceptor path (standalone `frdp-krb-authd` skeleton now base64-decodes its token argument before `gss_accept_sec_context()`, but `frdpd` still does not extract/pass real CredSSP SPNEGO tokens);
