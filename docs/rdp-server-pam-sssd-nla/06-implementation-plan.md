...
- [ ] SELinux/AppArmor profiles (draft examples install under `/usr/share/frdpd/security`, and the AppArmor example has parser validation when `apparmor_parser` is available; SELinux validation, activation, and production confinement review remain open);
