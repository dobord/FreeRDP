...
- [x] auth/session helper readiness and service recovery policy: explicit role-bound health responses, bounded pre-listen checks in `frdpd`, fail-closed requests during helper outage, `Restart=on-failure` units, and a live `frdp-authd` `SIGKILL`/same-socket restart test; in-flight PAM call interruption and full systemd integration testing remain broader deployment concerns;
