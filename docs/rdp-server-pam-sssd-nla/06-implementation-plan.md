...
- [x] auth/session helper readiness and service recovery policy: explicit role-bound health responses, bounded pre-listen checks in `frdpd`, fail-closed requests during helper outage, `Restart=on-failure` units, direct same-socket crash/restart coverage, deterministic in-flight PAM auth/session open/session close interruption coverage, and a root-gated transient-systemd test that kills and restarts both helper roles while requiring a new socket inode plus role-bound health on the same path; installed distro-unit execution and active-session recovery under the service manager remain deployment concerns;
