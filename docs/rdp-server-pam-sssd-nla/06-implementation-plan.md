...
- [ ] load testing harness (partial: `server/frdp/test/e2e/scripts/rdp-load-probe.sh` provides a configurable parallel auth-only RDP probe for retained E2E client environments, and `rdp-session-smoke.sh` covers one retained graphical lifecycle, with fail-fast retained-client tool checks; full graphical session load/soak coverage remains open);
