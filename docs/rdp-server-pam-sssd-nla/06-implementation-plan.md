...
- [ ] simple reconnect by user/session id (partial: reconnect-selection policy for matching authenticated user/UID identities to an explicit session id or the newest disconnected session has focused CTest coverage, normal peer disconnect sends a detach request that marks the in-memory session `DISCONNECTED`, authorized explicit session-id V3 requests attach to that disconnected entry, and the standard empty-session-id V3 login path implicitly attaches the newest unambiguous disconnected session for the authenticated user/UID before creating a new session; an Xvfb-backed live helper test now covers authorized V3 open, active listing, detach, disconnected listing, implicit reattach with stable session/display/socket/PID, hard close, and final process/socket cleanup when the host exposes the noninteractive PAM and Xvfb prerequisites; durable reconnect and RDP-client interoperability evidence remain open);
