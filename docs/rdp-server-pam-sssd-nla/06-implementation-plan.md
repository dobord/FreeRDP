...
- `server/frdp/frdp-session-agent` (launched by normal `frdp-sesmand` session requests; keyboard/mouse/Unicode BMP text backend injection, raw framebuffer tile capture with XDamage-backed dirty-tile tracking, and validated XRandR resize-request handling exist, but the current fixed-mode Xvfb backend cannot apply geometry changes, and IME/layout-safe text input and production channel features are not implemented yet);
