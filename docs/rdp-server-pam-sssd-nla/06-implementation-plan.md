...
- [x] real `frdp-authd --config` startup and `frdp-sesmand --config` startup/reread/apply path behind installed helper units and `frdpctl reload` for PAM service and session resource guard selection, with the reload response reporting the applied PAM service plus session resource guard values and failing closed on active Kerberos policy while that handler is unsupported; `frdpd` also handles `SIGHUP` by atomically publishing parsed `[channels]` and `[clipboard]` policy snapshots to future peers while active peers retain their connection-time policy and failed reloads retain the last valid policy;
- [ ] full runtime config reload coverage for listener sockets, TLS material, authentication/NTLM state, helper topology, and other non-channel fields; channel/clipboard policy reload is implemented only for future peers by design;
