...
- [ ] locked secret buffers and brokerized credential handling across the integrated auth/session path (normal startup now uses the auth broker, temporary password copies are locked/wiped, truncated auth-request decode clears caller-owned output before reading and authd wipes the full request after handling, PAM conversation response failure-cleanup is wiped, auth-token response/staging buffers are wiped after send/use, the peer-owned FreeRDP auth identity is cleared after authentication, Remote Credential Guard ticket-logon buffers are wiped before release, and the old direct PAM fallback has been removed; remaining CredSSP/PAM internals remain tracked).
