...
- [x] real-client local-PAM and Samba AD/SSSD profiles disable automatic client reconnect, verify fresh server-side evidence for three accepted NLA/PAM attempts, one wrong-password NTLM MIC rejection before PAM, and two named disabled-account PAM/SSSD denials from that probe's connection path, clean the observed session created by the successful auth-only probe, and prove each denied attempt leaves the managed-session registry plus durable session runtime directory empty;
