...
- shared IPC client operations use bounded socket send/receive timeouts, suppress `SIGPIPE` on disconnected peers, and use explicit little-endian/fixed-field wire formats for the common IPC header, auth broker request/response payloads, canonical V3 session-open handoff payload, and shared session response payload;
