...
- [x] provider-backed session-manager failure coverage when `libpam-wrapper` is available (test-only PAM modules record account/credential/session phases, deterministically reject or block `pam_open_session`, prove the real `frdp-sesmand` returns a clean V3 denial without creating a session, agent socket, display reservation, or durable metadata, prove an in-flight open client fails without pre-allocating those resources when the manager is killed, and prove a manager killed inside `pam_close_session` leaves bounded client failure plus durable `STOPPING` artifacts that a replacement manager reconciles);
...
- [x] cleanup on disconnect across `frdp-sesmand` agent process groups and PAM sessions (`session_socket` hard-close requests use covered cleanup-decision policy to terminate the agent process group, close PAM state, unlink runtime sockets, release display reservations, and mark lifecycle state through `ACTIVE` -> `STOPPING` -> `DEAD`; normal peer disconnect uses durable metadata for `DISCONNECTED` and falls back to a hard-close retry if the manager cannot acknowledge detach; Linux recovery binds agent PID/start time/effective UID/PGID plus agent-socket and display-reservation inode identities, and live local, Samba AD/SSSD, and joined FreeIPA/SSSD `SIGKILL`/restart tests prove process and runtime-artifact cleanup. `frdpd` tolerates two transient framebuffer IPC failures, keeps a lightweight agent probe active when RDP output is suppressed, and closes the stale peer on the third consecutive failure. A replacement manager cannot recover the lost PAM handle, so full PAM/logind reconciliation remains tracked separately).
