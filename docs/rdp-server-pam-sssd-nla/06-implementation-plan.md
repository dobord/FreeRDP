...
- a minimal framebuffer pump from `frdp-session-agent` to `frdpd`, with XDamage-backed dirty/clean-tile responses, per-peer unchanged-tile suppression, bounded tile pump scheduling, opportunistic minimum-color-loss/no-subsampling NSCodec `SurfaceBits` for negotiated smaller tiles with raw bitmap fallback, cache invalidation for refresh/suppress-output requests, and an internal monitor-layout callback to XRandR resize-request path that is not yet reachable through negotiated client Display Control transport.
...
- [ ] display resize (partial: an internal monitor-layout callback validates requests, forwards them to the agent, and updates peer geometry only after backend confirmation; the focused Xvfb smoke test exercises the control IPC at the existing geometry. Negotiated client transport is absent because `drdynvc` remains guard-denied and no Display Control DVC handler is wired, while the current Xvfb instance exposes only one fixed RandR mode, so actual client-driven geometry changes and resize churn remain open);
- [x] static channel policy engine (`frdpd` filters client-requested static virtual channels during capability processing with configurable blocklist/allowlist modes, defaults to empty blocklist mode, keeps `drdynvc` guard-denied until DVC transport and handlers are explicitly enabled, denies unimplemented static channels plus unsupported `rdpsnd`/`rdpdr` at the runtime-aware gate, and has CTest coverage for config parsing plus `CHANNEL_DEF` validation; useful channel handlers remain open);
