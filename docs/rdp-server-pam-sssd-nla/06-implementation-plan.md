...
- [ ] protocol regression suite (partial: `server/frdp/test/e2e/scripts/rdp-protocol-regression.sh` provides a retained-client auth-only NLA option matrix with per-case logs; broader full-session, codec/channel, reconnect, Windows-client, and sustained interop regression coverage remains open);
