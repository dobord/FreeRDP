...
- [ ] locked secret buffers and brokerized credential handling across the integrated auth/session path (normal startup now uses the auth broker, temporary password copies are locked/wiped, credential-bearing auth/session IPC decoders clear caller-owned outputs before reading, authd wipes the full request after handling, auth-token create/verify clear failed outputs plus local payload/HMAC/key material, PAM conversation response failure-cleanup is wiped, auth-token response/staging buffers are wiped after send/use, the peer-owned FreeRDP auth identity is cleared after authentication, Remote Credential Guard ticket-logon buffers are wiped before release, and the old direct PAM fallback has been removed; remaining CredSSP/PAM internals remain tracked).
