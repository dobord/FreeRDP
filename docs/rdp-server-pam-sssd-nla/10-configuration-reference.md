...
- Authentication failure limits are currently fixed broker policy rather than configuration keys: `frdp-authd` independently permits 10 credential/account denials for one normalized account and 10 for one remote source in a 120-second process-local window. Before PAM succeeds, account keys necessarily use the requested name, preferring its NSS/SSSD canonical name when already resolvable and using an ASCII case-folded fallback otherwise. PAM/NSS infrastructure errors fail closed without consuming denial budgets. The fixed 256-entry tables expire old windows and reject untrackable new identities fail closed. A successful login clears that pre-PAM account key but does not erase source failures. Broker restart resets these in-memory budgets; alias-coalescing after PAM and distributed/shared enforcement are not yet implemented.
