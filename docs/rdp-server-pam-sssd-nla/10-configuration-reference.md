...
- Authentication failure limits are currently fixed broker policy rather than configuration keys: `frdp-authd` independently permits 10 credential/account denials for one normalized account and 10 for one remote source in a 120-second process-local window. Account keys use the NSS/SSSD canonical name when resolvable and an ASCII case-folded fallback otherwise, preserving distinct case-sensitive accounts while preventing common unresolved-name case variants. PAM/NSS infrastructure errors fail closed without consuming denial budgets. The fixed 256-entry tables expire old windows and reject untrackable new identities fail closed. A successful login clears that account's failures but does not erase source failures. Broker restart resets these in-memory budgets; distributed/shared enforcement is not yet implemented.
