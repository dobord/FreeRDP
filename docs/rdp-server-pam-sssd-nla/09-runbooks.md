...
- **Blocked channels**: Channel filtering supports `blocklist`/`blacklist` and `allowlist`/`whitelist` modes. The default is empty blocklist mode; use `[channels].static_deny` to reject exact static names, or switch to static allowlist mode with `[channels].static_allow` when a restrictive deployment is required. `[channels].dynamic_deny` / `[channels].dynamic_allow` feed the DVC authorization callback, but remain preparatory while `drdynvc` and useful dynamic handlers stay disabled. Allowing a name does not by itself enable clipboard/audio/device support, and `rdpsnd` audio output plus `rdpdr` device redirection are runtime-denied until handlers and explicit policy exist.
