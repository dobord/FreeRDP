...
3. `frdpctl reload` asks `frdp-sesmand --config <path>` to reread `[auth].pam_service` and supported `[session]` resource guards for future session opens. Restart the affected daemon during a maintenance window for listener sockets, TLS material, channel policy, clipboard policy, and helper-topology changes.
...
- **Blocked channels**: Channel filtering supports `blocklist`/`blacklist` and `allowlist`/`whitelist` modes. The default is empty blocklist mode; use `[channels].static_deny` to reject exact static names, or switch to static allowlist mode with `[channels].static_allow` when a restrictive deployment is required. `[channels].dynamic_deny` / `[channels].dynamic_allow` feed the DVC authorization callback, but remain preparatory while `drdynvc` and useful dynamic handlers stay disabled. Allowing a name does not by itself enable clipboard/audio/device support; arbitrary static channels, `rdpsnd` audio output, and `rdpdr` device redirection are runtime-denied until handlers and explicit policy exist.
