...
5. Complete helper supervision and deterministic crash recovery. Agent
   ...
   systemd-logind registration remains open; optional transient per-session
   cgroup ownership is implemented.
...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and tri-state
  ...
  across manager restart is implemented; reconnect restoration and
  systemd-logind registration remain open, while optional transient
  systemd-scope cleanup is implemented;
