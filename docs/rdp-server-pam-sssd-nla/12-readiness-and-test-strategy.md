...
| Password-backed NLA and PAM | 63% | Integrated FreeRDP server callbacks, CredSSP identity extraction, PAM auth/account handling through the auth broker, focused user/domain normalization, non-interactive PAM conversation tests including auth-broker prompt rejection, deterministic PAM auth/account/session lifecycle harness coverage, locked temporary secret buffers, auth broker IPC required for normal startup, and peer-worker PAM fallback removed | Windows client and domain interoperability need repeatable evidence |
| Privilege-separated topology | 72% | `frdp-authd`, `frdp-sesmand`, per-user agent, Unix sockets, peer credential checks, process hardening, correlation IDs, normal startup requiring both helper sockets, signed single-use session-open auth tokens bound to POSIX uid/gid/bounded supplementary groups/account state, fixed-window per-peer helper IPC rate limits, live helper-topology startup smoke coverage, live-helper socket collision protection, explicit auth broker, session open/close, session list/reload response, and agent metadata IPC payload encoding, and no in-process PAM fallback | Legacy V1/V2 session-open message IDs remain compatibility residue but are rejected before body decode, frame pixel bytes remain a raw tail after explicit agent frame metadata, and the token does not yet carry a richer account-policy profile |
| Session lifecycle | 61% | PAM session ownership, covered root identity-drop ordering and fail-closed exact-identity validation for non-root same-user development sessions, Xvfb agent startup, a shared Linux process-identity reader for PID/start-time/owner-UID checks, FRDP-owned atomic display reservations with versioned PID/start-time identity and covered live/dead/PID-reuse/PID-1/legacy/same-inode reconciliation, explicit hard-close requests, normal disconnect detach into `DISCONNECTED`, in-memory state, cleanup-decision, disconnect-rollback and tri-state reconnect-selection coverage, authorized explicit session-id attach and implicit newest-disconnected attach bound to the authenticated user/UID identity, plus a prerequisite-gated live helper lifecycle covering V3 open/list/detach/implicit reattach/hard-close with stable session metadata and final process/socket cleanup, daemon-wide `TasksMax`, and configurable POSIX `RLIMIT_NPROC`/`RLIMIT_AS` guards for new session agents exist | No durable session registry, logind/cgroup ownership, full process/PAM restart reconciliation, cgroup CPU/memory accounting, production quota management, or real RDP-client reconnect evidence |
...
| Automated verification | 73% | Unit tests, helper-process component tests, live helper-topology startup smoke coverage, prerequisite-gated Xvfb/PAM live V3 open/list/detach/implicit-reconnect/hard-close process coverage, truncated boundary and slow complete auth/session helper client coverage, invalid session-open authorization coverage, in-memory session state transition, cleanup-decision and reconnect-selection coverage, agent resize and root-run frame-capture control-IPC smoke coverage, focused `server/frdp` ASan+UBSan, strict-warning, systemd unit verification plus hardening-directive contract checks, tmpfiles verification, monitoring textfile/alert-rule verification, E2E harness script/fixture validation with bounded timeout and explicit `xfreerdp`/`xvfb-run`/`Xvfb`/`nc`/`frdpctl` prerequisite contracts, config/channel-policy and fail-closed helper socket config coverage, auth-token, IPC codec, Kerberos no-core hardening, keytab-selection, acceptor-name, delegated-credential, bounded group lookup, token-decoding and principal-normalization coverage, frame-policy, input-policy, display-policy, session-lifecycle fuzzer, and CLI insecure session-socket refusal smoke coverage in the FRDP workflow, Docker Compose profiles for local PAM, Samba AD and FreeIPA, retained-client auth-only load and protocol option-matrix probe scripts, and preserved Compose/CTest/container diagnostic artifacts are present | Compose profiles must become green and stable; no Windows `mstsc` or real RDP-client reconnect, graphical session load/soak, sustained fuzz corpus, broader selected RDP input fuzzing, full-session protocol regression, Valgrind/LSan variant matrix or crash-recovery gates yet |
...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and tri-state
  ...
  with stable session metadata, while real-client reconnect evidence and
  logind/cgroup cleanup adapters remain open;
