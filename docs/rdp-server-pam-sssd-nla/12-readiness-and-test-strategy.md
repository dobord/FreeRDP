...
session properties before import. Installed provider/Windows-client manager-restart
...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and tri-state
  ...
  and plain, transient-scoped, plus explicit-login1 disconnected reconnect restoration across manager restart are
  implemented; installed-provider restoration remains open,
...
- provider-backed and Windows-client reconnect across `frdp-sesmand` restart;
