...
5. Complete helper supervision and deterministic crash recovery. Agent
   ...
   recovery, and active-session Samba AD/SSSD plus FreeIPA/SSSD manager crashes
   with bounded stale-peer disconnect plus fresh provider-backed login are covered. Lost
...
profile requires one wrong-password NTLM MIC rejection before PAM, two named
denied-user PAM/SSSD outcomes from that probe's connection path, and no NTLM
