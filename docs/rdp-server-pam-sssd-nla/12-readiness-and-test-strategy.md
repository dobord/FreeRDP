...
| Session lifecycle | 50% | PAM session ownership, uid/gid drop, Xvfb agent startup, FRDP-owned atomic display reservation files with stale-PID reservation reconciliation, close requests, in-memory state, cleanup-decision and reconnect-selection coverage, authorized explicit session-id attach to disconnected in-memory sessions, cleanup, daemon-wide `TasksMax`, and configurable POSIX `RLIMIT_NPROC`/`RLIMIT_AS` guards for new session agents exist | No normal disconnect detach/reconnect flow, durable registry, logind/cgroup ownership, full restart reconciliation, cgroup CPU/memory accounting, or production quota management |
...
- in-memory session-state transition, cleanup-decision and reconnect-selection
  policy now have focused CTest coverage, and authorized explicit session-id
  requests can attach to a disconnected in-memory session; normal disconnect
  detach/reconnect flow and logind/cgroup cleanup adapters remain open;
