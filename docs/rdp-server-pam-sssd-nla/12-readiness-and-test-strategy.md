...
| Operations and packaging | 33% | Example systemd units, PAM file, configuration, install rules, draft MAC policy files with SELinux module/package and AppArmor parser validation when available, starter node_exporter/Prometheus monitoring examples, Debian and RPM preview package smoke evidence, and package signing/reproducible-build notes exist | No dependency-checked target-distro RPM CI, release signing repository, active SELinux/AppArmor enforcement review, upgrade/rollback, socket activation, Grafana dashboards, native metrics or operational SLOs |
| Automated verification | 64% | Unit tests, helper-process component tests, live helper-topology startup smoke coverage, truncated boundary and slow complete auth/session helper client coverage, invalid session-open authorization coverage, in-memory session state transition, cleanup-decision and reconnect-selection coverage, agent resize and root-run frame-capture control-IPC smoke coverage, focused `server/frdp` ASan+UBSan, strict-warning, systemd unit verification, config/channel-policy, auth-token, IPC codec, frame-policy, input-policy, display-policy, session-lifecycle fuzzer, and CLI insecure session-socket refusal smoke coverage in the FRDP workflow, Docker Compose profiles for local PAM, Samba AD and FreeIPA, retained-client auth-only load and protocol option-matrix probe scripts, and preserved Compose/CTest/container diagnostic artifacts are present | Compose profiles must become green and stable; no Windows `mstsc`, runtime reconnect, graphical session load/soak, sustained fuzz corpus, broader selected RDP input fuzzing, full-session protocol regression, Valgrind/LSan variant matrix or crash-recovery gates yet |
...
- SELinux module/package validation and AppArmor parser validation are now covered when the host tools are available; sanitizer and Valgrind/LSan variants remain open.
