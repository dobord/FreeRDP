...
| Operations and packaging | 50% | Example systemd/PAM/config/install assets, focused draft MAC and starter monitoring validation, clean Ubuntu 24.04 Debian plus Fedora 42 RPM declared-dependency build/install/private-linkage runs, scoped Debian runtime DEP-5 metadata, an error-level lintian gate, a verified no-enable/no-start policy, and a privileged systemd package install/start/upgrade/rollback/remove/purge gate with provisioned TLS/SAM, real packaged daemon PIDs, role-bound authd startup health, sesmand control IPC, listener checks, and independent Ubuntu FreeRDP 3.5 local-PAM session/reconnect evidence at base/upgrade/rollback exist | Successful package-job history, complete source copyright and non-error lintian review, broader distro/architecture coverage, release signing repository, active MAC enforcement review, installed-package SSSD-provider and active-session upgrade evidence, socket activation, dashboard import evidence, native metrics, and operational SLOs remain absent |
