...
| Operations and packaging | 29% | Example systemd units, PAM file, configuration, install rules, draft MAC policy files with AppArmor parser validation when available, starter node_exporter/Prometheus monitoring examples, Debian preview package smoke evidence, and package signing/reproducible-build notes exist | No completed RPM build, release signing repository, active SELinux validation, active AppArmor enforcement review, upgrade/rollback, socket activation, Grafana dashboards, native metrics or operational SLOs |
...
- AppArmor parser validation is now covered when `apparmor_parser` is available; SELinux policy validation, sanitizer and Valgrind/LSan variants remain open.
