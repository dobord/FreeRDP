...
| Operations and packaging | 28% | Example systemd units, PAM file, configuration, install rules, draft MAC policy files, starter node_exporter/Prometheus monitoring examples, Debian preview package smoke evidence, and package signing/reproducible-build notes exist | No completed RPM build, release signing repository, active SELinux/AppArmor validation, upgrade/rollback, socket activation, Grafana dashboards, native metrics or operational SLOs |
