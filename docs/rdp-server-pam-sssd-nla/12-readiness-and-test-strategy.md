...
| Automated verification | 59% | Unit tests, helper-process component tests, live helper-topology startup smoke coverage, truncated and slow complete auth/session helper client coverage, invalid session-open authorization coverage, agent resize control-IPC smoke coverage, focused `server/frdp` ASan+UBSan, strict-warning, config/channel-policy, auth-token, and frame-policy fuzzer smoke coverage in the FRDP workflow, Docker Compose profiles for local PAM, Samba AD and FreeIPA, and preserved Compose/CTest/container diagnostic artifacts are present | Compose profiles must become green and stable; no Windows `mstsc`, reconnect, load, sustained fuzz corpus, broader selected RDP input fuzzing, Valgrind/LSan variant matrix or crash-recovery gates yet |
