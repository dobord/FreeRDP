...
5. Complete helper supervision and deterministic crash recovery. Agent
   ...
   Per-session PAM owners, metadata V3, synchronized close receipts, stale
...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and tri-state
  ...
  across manager restart is implemented; reconnect restoration remains open,
  while optional explicit login1 registration and transient systemd-scope cleanup
  are implemented;
...
- reconnect across `frdp-sesmand` restart on top of the implemented PAM-handle cleanup reconciliation;
