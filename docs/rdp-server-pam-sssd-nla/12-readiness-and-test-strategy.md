...
handoff payloads, session close requests, and shared session responses now use
explicit little-endian/fixed-field wire formats, while legacy session open/V2,
session list/reload/control, and agent payload serialization/versioning plus
richer account-policy payloads remain open.
