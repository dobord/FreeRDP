...
Controlled manager-crash recovery and reconnect are now implemented for plain and
transient-scoped `DISCONNECTED` sessions. Metadata V4 binds the exact PAM/SSSD
...
and hard close. Explicit-login1 import and installed provider/Windows-client
...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and tri-state
  ...
  and plain plus transient-scoped disconnected reconnect restoration across manager restart are
  implemented; login1 and installed-provider restoration remain open,
...
- provider-backed and Windows-client reconnect across `frdp-sesmand` restart, plus explicit-login1 session import;
