...
| Operations and packaging | 43% | Example systemd/PAM/config/install assets, focused draft MAC and starter monitoring validation, clean Ubuntu 24.04 Debian plus Fedora 42 RPM declared-dependency build/install/private-linkage runs, scoped Debian runtime DEP-5 metadata and an error-level lintian policy gate with failure-report retention, matching package CI jobs, and signing/reproducible-build guidance exist | Successful package-job history, complete source copyright and non-error lintian review, broader distro/architecture coverage, service enablement policy, release signing repository, active MAC enforcement review, upgrade/rollback, socket activation, dashboard import evidence, native metrics, and operational SLOs remain absent |
