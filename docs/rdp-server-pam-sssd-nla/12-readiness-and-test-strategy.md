...
| Kerberos-first path | 17% | A build-only GSSAPI helper skeleton, standalone keytab environment selection, host-based acceptor-name import, delegated-credential flag rejection, base64 token decoding, fail-closed simple principal normalization and bounded group lookup coverage, and architecture documentation exist | No CredSSP/SPNEGO token transport from `frdpd`, daemon-enforced Kerberos config fields, SSSD enterprise principal mapping, account/session binding or integrated security review |
