...
| Operations and packaging | 46% | Example systemd/PAM/config/install assets, focused draft MAC and starter monitoring validation, clean Ubuntu 24.04 Debian plus Fedora 42 RPM declared-dependency build/install/private-linkage runs, scoped Debian runtime DEP-5 metadata, an error-level lintian gate, a verified no-enable/no-start policy, and a privileged systemd package install/start/upgrade/rollback/remove/purge gate exist | Successful package-job history, complete source copyright and non-error lintian review, broader distro/architecture coverage, release signing repository, active MAC enforcement review, real-daemon upgrade/rollback, socket activation, dashboard import evidence, native metrics, and operational SLOs remain absent |
