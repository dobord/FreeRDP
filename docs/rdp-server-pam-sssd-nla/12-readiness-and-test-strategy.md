...
| Session lifecycle | 52% | PAM session ownership, uid/gid drop, Xvfb agent startup, FRDP-owned atomic display reservation files with stale-PID reservation reconciliation, explicit hard-close requests, normal disconnect detach into `DISCONNECTED`, in-memory state, cleanup-decision, disconnect-rollback and reconnect-selection coverage, authorized explicit session-id attach to disconnected in-memory sessions, cleanup, daemon-wide `TasksMax`, and configurable POSIX `RLIMIT_NPROC`/`RLIMIT_AS` guards for new session agents exist | No implicit user reconnect selection in `frdpd`, durable registry, logind/cgroup ownership, full restart reconciliation, cgroup CPU/memory accounting, production quota management, or real-client reconnect evidence |
...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and reconnect-selection
  policy now have focused CTest coverage, normal disconnect detaches to a
  disconnected in-memory session, and authorized explicit session-id requests
  can attach back to it; implicit reconnect policy, real-client reconnect
  evidence and logind/cgroup cleanup adapters remain open;
