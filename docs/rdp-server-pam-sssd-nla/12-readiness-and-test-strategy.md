...
| Operations and packaging | 27% | Example systemd units, PAM file, configuration, install rules, draft MAC policy files, Debian preview package smoke evidence, and package signing/reproducible-build notes exist | No completed RPM build, release signing repository, active SELinux/AppArmor validation, upgrade/rollback, socket activation, metrics or operational SLOs |
| Automated verification | 63% | Unit tests, helper-process component tests, live helper-topology startup smoke coverage, truncated boundary and slow complete auth/session helper client coverage, invalid session-open authorization coverage, in-memory session state transition, cleanup-decision and reconnect-selection coverage, agent resize and root-run frame-capture control-IPC smoke coverage, focused `server/frdp` ASan+UBSan, strict-warning, systemd unit verification, config/channel-policy, auth-token, IPC codec, frame-policy, input-policy, display-policy, session-lifecycle fuzzer, and CLI insecure session-socket refusal smoke coverage in the FRDP workflow, Docker Compose profiles for local PAM, Samba AD and FreeIPA, a retained-client auth-only load probe script, and preserved Compose/CTest/container diagnostic artifacts are present | Compose profiles must become green and stable; no Windows `mstsc`, runtime reconnect, graphical session load/soak, sustained fuzz corpus, broader selected RDP input fuzzing, Valgrind/LSan variant matrix or crash-recovery gates yet |
