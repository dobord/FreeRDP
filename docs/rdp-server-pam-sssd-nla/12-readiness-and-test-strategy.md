...
handoff payloads, and shared session responses now use explicit
little-endian/fixed-field wire formats, while legacy session close/open, session
list/reload/control, and agent payload serialization/versioning plus richer
account-policy payloads remain open.
