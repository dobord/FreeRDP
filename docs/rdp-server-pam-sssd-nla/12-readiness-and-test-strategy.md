...
| Kerberos-first path | 14% | A build-only GSSAPI helper skeleton, standalone keytab environment selection, base64 token decoding, fail-closed simple principal normalization coverage, and architecture documentation exist | No CredSSP/SPNEGO token transport from `frdpd`, configured SPN validation, SSSD enterprise principal mapping, account/session binding or security review |
