...
| Password-backed NLA and PAM | 72% | Integrated FreeRDP server callbacks, build-time optional (default-on) NTLM proof verification through a protected WinPR SAM, proof-to-delegated-credential identity binding, mandatory PAM auth/account handling through the auth broker after NTLM, focused user/domain normalization, non-interactive PAM conversation tests including auth-broker prompt rejection, deterministic PAM auth/account/session lifecycle harness coverage, locked temporary secret buffers, auth broker IPC required for normal startup, peer-worker PAM fallback removed, and green real-client local-PAM plus Samba AD/SSSD Compose profiles | SAM provisioning remains a separate password-equivalent secret store; Windows `mstsc`, Kerberos-first, and broader domain-policy interoperability evidence remain absent |
...
| Desktop data path | 31% | Input injection, raw/XDamage tile capture, bounded output scheduling, an internal resize callback and agent IPC path, same-geometry agent IPC smoke coverage, root-run forced framebuffer tile capture smoke coverage, and opportunistic NSCodec exist | Negotiated Display Control transport is absent because `drdynvc` is guard-denied and no handler is wired; the current Xvfb backend also exposes one fixed RandR mode and cannot change geometry; no production RFX/RDPGFX policy, alternate resizable backend, systematic performance, or real-client resolution interoperability evidence exists |
