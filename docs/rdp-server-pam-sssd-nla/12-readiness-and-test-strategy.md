...
handoff payloads, session close requests, shared session responses, and session
list/reload responses now use explicit little-endian/fixed-field wire formats,
while legacy session open/V2 and agent payload serialization/versioning plus
richer account-policy payloads remain open. Session list/reload requests are
header-only control messages.
