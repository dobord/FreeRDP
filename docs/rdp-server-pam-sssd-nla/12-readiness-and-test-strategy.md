...
| Privilege-separated topology | 58% | `frdp-authd`, `frdp-sesmand`, per-user agent, Unix sockets, peer credential checks, process hardening, correlation IDs, normal startup requiring both helper sockets, and no default in-process PAM fallback | Authenticated identity is passed as mutable strings without a cryptographically bound, single-use authorization object |
