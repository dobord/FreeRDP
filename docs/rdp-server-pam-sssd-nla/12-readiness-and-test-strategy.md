...
| Password-backed NLA and PAM | 55% | Integrated FreeRDP server callbacks, CredSSP identity extraction, PAM auth/account handling, locked temporary secret buffers, optional auth broker IPC | The broker path is optional; credentials can still be processed by the peer worker; Windows client and domain interoperability need repeatable evidence |
| Privilege-separated topology | 50% | `frdp-authd`, `frdp-sesmand`, per-user agent, Unix sockets, peer credential checks, process hardening and correlation IDs exist | The topology is not the runtime default; authenticated identity is passed as mutable strings without a cryptographically bound, single-use authorization object |
