...
| Session lifecycle | 60% | PAM session ownership, covered root identity-drop ordering and fail-closed exact-identity validation for non-root same-user development sessions, Xvfb agent startup, FRDP-owned atomic display reservation files with stale-PID reservation reconciliation, explicit hard-close requests, normal disconnect detach into `DISCONNECTED`, in-memory state, cleanup-decision, disconnect-rollback and tri-state reconnect-selection coverage, authorized explicit session-id attach and implicit newest-disconnected attach bound to the authenticated user/UID identity, plus a prerequisite-gated live helper lifecycle covering V3 open/list/detach/implicit reattach/hard-close with stable session metadata and final process/socket cleanup, daemon-wide `TasksMax`, and configurable POSIX `RLIMIT_NPROC`/`RLIMIT_AS` guards for new session agents exist | No durable registry, logind/cgroup ownership, full restart reconciliation, cgroup CPU/memory accounting, production quota management, or real RDP-client reconnect evidence |
