...
| Kerberos-first path | 12% | A build-only GSSAPI helper skeleton, fail-closed simple principal normalization coverage, and architecture documentation exist | No CredSSP/SPNEGO token transport, configured SPN validation, SSSD enterprise principal mapping, account/session binding or security review |
