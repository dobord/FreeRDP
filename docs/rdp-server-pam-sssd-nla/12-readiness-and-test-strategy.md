...
| Operations and packaging | 42% | Example systemd/PAM/config/install assets, focused draft MAC and starter monitoring validation, clean Ubuntu 24.04 Debian plus Fedora 42 RPM declared-dependency build/install/private-linkage runs, matching package CI jobs, and signing/reproducible-build guidance exist | Successful package-job history, broader distro/architecture coverage, lintian/copyright and service enablement policy, release signing repository, active MAC enforcement review, upgrade/rollback, socket activation, dashboard import evidence, native metrics, and operational SLOs remain absent |
