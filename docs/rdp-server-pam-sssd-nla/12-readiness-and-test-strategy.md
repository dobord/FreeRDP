...
| Password-backed NLA and PAM | 58% | Integrated FreeRDP server callbacks, CredSSP identity extraction, PAM auth/account handling, locked temporary secret buffers, auth broker IPC required for normal startup | Credentials can still be processed by the peer worker through an explicit legacy flag; Windows client and domain interoperability need repeatable evidence |
| Privilege-separated topology | 55% | `frdp-authd`, `frdp-sesmand`, per-user agent, Unix sockets, peer credential checks, process hardening, correlation IDs, and normal startup requiring both helper sockets exist | Authenticated identity is passed as mutable strings without a cryptographically bound, single-use authorization object, and the legacy in-process fallback still exists |
...
| Automated verification | 45% | Unit tests, helper-process component tests and Docker Compose profiles for local PAM, Samba AD and FreeIPA are present | Compose profiles must become green and stable; no Windows `mstsc`, reconnect, load, fuzzing, leak or crash-recovery gates yet |
