...
| Automated verification | 66% | Unit tests, helper-process component tests, live helper-topology startup smoke coverage, truncated boundary and slow complete auth/session helper client coverage, invalid session-open authorization coverage, in-memory session state transition, cleanup-decision and reconnect-selection coverage, agent resize and root-run frame-capture control-IPC smoke coverage, focused `server/frdp` ASan+UBSan, strict-warning, systemd unit and tmpfiles verification, monitoring textfile/alert-rule verification, config/channel-policy, auth-token, IPC codec, frame-policy, input-policy, display-policy, session-lifecycle fuzzer, and CLI insecure session-socket refusal smoke coverage in the FRDP workflow, Docker Compose profiles for local PAM, Samba AD and FreeIPA, retained-client auth-only load and protocol option-matrix probe scripts, and preserved Compose/CTest/container diagnostic artifacts are present | Compose profiles must become green and stable; no Windows `mstsc`, runtime reconnect, graphical session load/soak, sustained fuzz corpus, broader selected RDP input fuzzing, full-session protocol regression, Valgrind/LSan variant matrix or crash-recovery gates yet |
