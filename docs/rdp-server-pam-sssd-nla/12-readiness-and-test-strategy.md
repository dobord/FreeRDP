...
| Session lifecycle | 66% | PAM session ownership, covered identity drop, Xvfb agent startup, shared PID/start-time/effective-UID identity reads, versioned atomic session metadata committed before lifecycle responses, durable detach/reconnect state updates, inode-bound agent-socket/display-reservation cleanup, and a live `SIGKILL`/same-runtime restart gate using pidfd-directed agent shutdown plus process-group disappearance now exist alongside hard-close and reconnect coverage | Restart intentionally cleans rather than restores sessions; the replacement process cannot close the lost PAM handle, and logind/cgroup ownership, full PAM reconciliation, production quota management, and real RDP-client reconnect evidence remain absent |
