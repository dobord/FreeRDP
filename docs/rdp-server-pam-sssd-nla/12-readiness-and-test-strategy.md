...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and tri-state
  ...
  and plain disconnected reconnect restoration across manager restart are
  implemented; scoped/login1 and installed-provider restoration remain open,
...
- provider-backed and Windows-client reconnect across `frdp-sesmand` restart, plus scoped/login1 session import;
