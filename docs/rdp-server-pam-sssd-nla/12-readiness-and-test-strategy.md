...
| Privilege-separated topology | 93% | `frdp-authd`, `frdp-sesmand`, per-user agent, Unix sockets, peer credential checks including root-gated live cross-UID rejection and post-rejection helper survival for both helper roles, process hardening, correlation IDs, role-bound bounded startup health checks, fail-closed helper outages, restart-on-failure unit policy, direct crash/restart coverage, root-gated transient-systemd recovery for both helper roles, installed Debian-unit disconnected-session restart/reconnect, in-flight-auth crash, and bounded-outage recovery, installed-package Samba AD/SSSD recovery with a stable provider service and join, and Samba AD/SSSD plus joined FreeIPA/SSSD active-session manager crashes with exact endpoint replacement, stale-peer disconnect, old-agent/runtime cleanup and fresh provider-backed relogin/reconnect. Signed single-use session-open auth tokens bind POSIX uid/gid/bounded supplementary groups/account state; fixed-window per-peer helper IPC rate limits, live-helper socket collision protection, explicit auth broker and session control responses exist; no in-process PAM fallback remains | Longer operational outage policy, legacy V1/V2 compatibility IDs, and richer account-policy payload remain open |
| Session lifecycle | 91% | Per-session PAM ownership retains `pam_handle_t` across manager failure, writes synchronized close receipts, and binds recovery to metadata V4. After `pidfd` confirms manager death, the owner permits exactly one same-UID replacement manager takeover within 10 seconds, pins all commands to that PID, idempotently confirms its retry, rejects concurrent and chained takeover, bounds incomplete commands, and keeps retrying agent termination immediately after the replacement dies. Integration coverage exercises successful and repeated takeover, rejected takeover and login1-FIFO binding from another PID, a stalled same-UID client, chained-takeover rejection, and replacement-death cleanup. Restart reconciliation takes ownership before potentially slow scope/process teardown. Provisional artifact ownership is committed as `STARTING` before `fork()`, PID/start-time identity replaces it while the child remains behind the launch barrier, `ACTIVE` follows ready/UID verification, and receipts remain until artifact plus metadata removal succeeds. Receipt-backed orphan socket cleanup and PID/start-time reservation reconciliation cover the pre-metadata artifact window; pidfd-directed stop pins the complete PGID through TERM/KILL owner cleanup. Failed close markers and stale endpoints without receipts block startup. Covered identity drop, Xvfb agent startup, shared process-identity reads, durable detach/reconnect state updates, inode-bound cleanup, bounded agent heartbeat supervision, optional transient per-session systemd scopes with accounting and process/memory/CPU limits, atomic per-session runtime quota updates, rollback-protected config reload/reset, D-Bus reconnection, metadata-driven restart cleanup, cgroup-v2 fallback, and root/systemd-gated property plus detached-descendant lifecycle tests exist. Explicit login1 mode drops the blocked child to its final UID before PIDFD registration; after owner takeover the replacement manager obtains a duplicated lifetime FIFO only through the PID-pinned control channel and verifies the authoritative PID/session, UID, user, service, remote X11 class/type/display, leader, and runtime path. The root-gated `TestFreeRDPFrdpSesmandLogindLifecycle` CTest enters a transient system unit and proves stable login1 ID and agent identity through detach, manager `SIGKILL`, restart import, reconnect, and hard cleanup. Installed Ubuntu local-PAM and Samba AD/SSSD lifecycle gates additionally detach the client, kill the packaged manager, import and reconnect the same session/display/agent/login1 identity, then prove explicit final cleanup. The same gates make an active manager ignore `SIGTERM`, require the shipped 30-second `TimeoutStopSec` to end an ordinary stop with `Result=timeout`, and verify PAM-owner, agent process-group, login1, and runtime cleanup under `KillMode=process` before restart and relogin. Provider-backed manager crashes inside PAM open/close prove bounded clients and fail-closed preservation when open or close cannot be confirmed; confirmed post-launch close recovery removes runtime artifacts. Live stopped-agent and local/provider `SIGKILL` cleanup gates, bounded `frdpd` stale-peer disconnect, and real `xfreerdp` reconnect/provider recovery cycles also exist. The local profile gracefully restarts `frdpd`, drains its peer workers, preserves the PAM-owned manager/agent session, and proves stable reattach | Controlled plain, transient-scoped, explicit-login1, installed local-PAM, and installed Samba AD/SSSD `DISCONNECTED` crash import, recovered heartbeat, reservation transfer, reconnect, and packaged forced-stop cleanup are covered; broader provider/distribution and Windows-client evidence remain absent |
...
Controlled manager-crash recovery and reconnect are now implemented for plain,
transient-scoped, and explicit-login1 `DISCONNECTED` sessions. Metadata V4 binds the exact PAM/SSSD
account name and UID; startup
...
and hard cleanup. Scoped recovery additionally verifies the active deterministic
unit and exact agent cgroup, restores finite or unlimited runtime quotas, and is
covered through manager `SIGKILL`, reconnect, post-import updates, reload reset,
and hard close. Login1 recovery additionally transfers its retained lifetime FIFO
from the takeover-pinned PAM owner and validates authoritative login1 identity and
session properties before import. Installed Ubuntu local-PAM and Samba AD/SSSD
package gates now repeat detach, automatic unit restart, stable session/display/agent/login1
identity through reconnect, and explicit final cleanup. Broader provider/distribution
and Windows-client manager-restart evidence remains open.
...
3. Implement reconnect policy and prove that reconnect attaches to exactly the
   intended session.
4. Retain the completed optional transient-scope quota policy: cgroup process
   ownership/cleanup, `TasksMax`, `MemoryMax`, CPU quota, atomic per-session
   updates, and rollback-protected config reload/reset all have focused and
   root/systemd live coverage. Extend this only when production load evidence
   justifies additional controls.
5. Complete helper supervision and deterministic crash recovery. Agent
   ...
   stale-peer disconnect plus fresh provider-backed login are covered.
   Per-session PAM owners, metadata V4, synchronized close receipts, stale
   endpoint cleanup, and fail-closed unconfirmed-close recovery are implemented.
   Optional explicit systemd-logind registration and transient per-session
   cgroup ownership are implemented. Root transient-service and installed local-PAM plus
   Samba AD/SSSD login1 crash cleanup are covered.
...
- in-memory session-state transition, cleanup-decision, disconnect-rollback and tri-state
  ...
  require one stable session id/display/agent PID. PAM cleanup reconciliation
  and plain, transient-scoped, explicit-login1, installed local-PAM, plus installed
  Samba AD/SSSD disconnected reconnect restoration across manager restart are implemented,
  while optional explicit login1 registration and transient systemd-scope
...
`pam_close_session()`, and proves the replacement manager refuses startup while
the close result is unconfirmed and preserves durable `STOPPING` state plus
runtime artifacts. The normal crash path proves the surviving per-session owner
closes PAM, records a receipt, and permits bounded restart reconciliation.
Extend it with:
...
- additional installed provider/distribution and Windows-client reconnect across `frdp-sesmand` restart;
