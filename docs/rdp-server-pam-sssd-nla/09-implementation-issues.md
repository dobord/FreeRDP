...
The integrated `server/frdpd` target builds successfully and now includes the partial configuration
parser. The standalone prototype sources in `/frdp-authd`, `/frdp-sesmand`, `/frdp-session-agent`,
`/frdp-krb-authd`, and `tools/frdpctl` are not part of that build or the canonical runtime topology.
...
| FRDP-008 | `server/frdpd` consumes only a subset of `frdpd.toml`. | 55% | 100% | `server/frdpd/frdpd.c` applies `listen`, implemented `security=nla`, `tls_cert`, `tls_key`, `mode=pam-sssd`, and `pam_service`; `config/frdp-config.c` rejects unsupported Kerberos/NTLM, `max_connections`, `session`, `channels`, and `audit` fields. | Kerberos policy, connection caps, sessions, channel policy, and audit config remain unavailable until enforcement exists, but they no longer appear as silently accepted active policy. | Extend config parsing and application incrementally, failing closed on invalid values and documenting unsupported fields until they are wired. |
| FRDP-009 | Packaging files still reference helper binaries that are not built or installed by CMake. | 70% | 100% | `packaging/rpm/frdpd.spec:37-42` lists helper binaries and `frdpctl` that are not built by CMake; only `server/frdpd` is integrated as a target. | Packages are likely to fail or install incomplete helper topology. | Add real CMake targets/install rules for helpers or remove them from package manifests until integrated. |
| FRDP-010 | `frdp-authd` still has test-only command-line password ingress. | 70% | 100% | `frdp-authd/frdp-authd.c` still accepts `<username> <password>` positional arguments outside IPC mode. | Passwords can leak through process listings, shell history, crash telemetry, and audit systems if the test path is installed or used operationally. | Remove argv password support from production builds, or gate it behind an explicit test-only build option not installed in packages. |
