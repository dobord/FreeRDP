...
| FRDP-009 | Packaging remains draft-only and is not build-verified. | 55% | 100% | `server/frdp/CMakeLists.txt` now installs the listed runtime helper binaries, `frdpd.toml`, and the `frdpd` PAM service example; no RPM/DEB package build has been run, and systemd/security policy installation is still incomplete. | Packages can still fail in CI or install a daemon without the required systemd/security policy files. | Add package build verification and install rules for systemd/security policy artifacts. |
