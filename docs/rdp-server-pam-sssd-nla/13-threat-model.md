...
| Display/session collision or stale cleanup | FRDP-owned display reservation files with versioned PID/start-time identity, shared same-proc-entry PID/start-time/owner-UID reads, live/dead/PID-1/PID-reuse reconciliation, safe same-inode release policy tests, process-group cleanup | Persistent session registry, full process/PAM restart reconciliation, logind/cgroup ownership |
