...
| Display/session collision or stale cleanup | FRDP-owned display reservations, versioned atomic metadata with V2 scope ownership, V3 PAM ownership, V4 exact account identity and V1-V3 cleanup compatibility, provisional pre-fork artifact ownership, pidfd-pinned PID/start-time/effective-UID plus PGID verification for non-scoped sessions, pidfd-directed leader stop while signaling a group, inode-bound socket/reservation cleanup, receipt-backed orphan socket cleanup, global PID/start-time reservation reconciliation, fail-closed malformed state, per-session PAM owners with synchronized close receipts and live/uncertain endpoint rejection, one deadline-bound same-UID replacement-manager takeover after old-manager `pidfd` death with all-command PID pinning, same-PID idempotence, concurrent/chained-takeover rejection, short incomplete-command deadlines and replacement-death cleanup retries, a live sesmand `SIGKILL`/restart reconciliation gate, optional fail-closed transient systemd scopes with atomic per-session quota updates and reload reset, and alternative explicit login1 registration whose FIFO is retained by the PAM owner across manager failure and transferred only to its takeover-pinned replacement after authoritative login1 PID/session/identity/property checks | Controlled plain, transient-scoped, explicit-login1, installed local-PAM, and installed Samba AD/SSSD `DISCONNECTED` import/reconnect preserve the control heartbeat and inode-pinned reservation; broader provider/distribution and Windows-client evidence remains required, and an indefinitely blocked PAM close intentionally blocks reconciliation rather than claiming cleanup |
...
| Audit log injection or correlation loss | Correlation ids across auth/session/agent path, escaping for textual client/IPC fields, and fixed-field journald events for configured channel authorization/activation and resize decisions | Move helper lifecycle events to the common structured emitter and add installed journald query evidence |
| Configuration drift or unsafe reload | Fail-closed parsing for unsupported policy, `frdpctl reload` for session PAM-service reread, and atomic `frdpd` `SIGHUP` publication of config-sourced peer admission plus channel/clipboard/audit policy with CLI override priority, existing-peer retention, and last-valid retention on parse failure | Runtime reload policy for listener sockets, TLS material, authentication/NTLM state, helper topology, and other non-policy fields |
