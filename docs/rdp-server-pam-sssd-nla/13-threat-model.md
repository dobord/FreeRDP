...
| Display/session collision or stale cleanup | FRDP-owned display reservation files with versioned PID/start-time identity, disappeared-owner and PID-reuse reconciliation, safe same-inode release policy tests, process-group cleanup | Persistent session registry, full process/PAM restart reconciliation, logind/cgroup ownership |
