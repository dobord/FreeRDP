...
| Configuration drift or unsafe reload | Fail-closed parsing for unsupported policy, `frdpctl reload` for session PAM-service reread, and atomic `frdpd` `SIGHUP` publication of channel/clipboard snapshots to future peers with last-valid retention on parse failure | Runtime reload policy for listener sockets, TLS material, authentication/NTLM state, helper topology, and other non-channel fields |
