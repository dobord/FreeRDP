...
| Substitution between the NLA proof identity and delegated password credentials | Bounded comparison against the configured PAM-normalized identity, with ASCII-case-insensitive equivalent split/down-level/UPN syntax accepted and embedded data, a domain-bearing proof after plain-mode reduction to a local PAM identity, explicit-domain separators, empty, mixed, repeated, unterminated, or differently named domains rejected fail closed; a domainless proof may bind that final local identity | Add Unicode-aware case folding and explicit trusted NetBIOS-to-DNS domain mapping only with reviewed configuration and Windows-client interoperability evidence |
