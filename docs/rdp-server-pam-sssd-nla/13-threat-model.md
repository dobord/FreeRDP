...
| Display/session collision or stale cleanup | FRDP-owned display reservations, versioned atomic metadata with V2 scope ownership and V1 compatibility, pidfd-pinned PID/start-time/effective-UID plus PGID verification for non-scoped sessions, inode-bound socket/reservation cleanup, fail-closed malformed state, a live sesmand `SIGKILL`/restart reconciliation gate, and optional fail-closed transient systemd scopes that confirm complete pre-exec child cgroup ownership with `TasksMax`/`MemoryMax`/CPU quota, survive process-group escape, accept rollback-protected bulk limit reload, and are stopped during normal or metadata-driven restart cleanup | Restart cleans rather than restores sessions; lost PAM handles cannot be closed by the replacement manager, and systemd-logind/PAM reconciliation plus individual runtime quota policy remain required |
